/**
 * homelab-hub — application logic (data validation, probing, HTTP handlers).
 *
 * Split out of server.mjs so it can be imported and unit-tested without the side effects
 * of server.mjs's bootstrap (opening a listening socket, wiring signal handlers, etc.) -
 * importing this module does nothing but define functions and module-level state.
 * server.mjs is now the thin bootstrap: it imports requestHandler/loadData/sweep from here
 * and does the actual `server.listen()`.
 *
...
export const SWEEP_INTERVAL_MS = Number(process.env.SWEEP_INTERVAL_MS || 15000);
...
// Read lazily (not cached at import time) so tests can flip process.env.READ_ONLY without
// needing to reset the module registry - the env var never changes during a real process
// lifetime anyway, so this is behaviorally identical in production.
...
/** Current in-memory document (null until loadData()/setData() has run). */
export function getData() {
  return data;
}

/** Test/bootstrap hook to inject the in-memory document directly. */
...
export const statuses = new Map(); // link id -> LinkStatus
let lastSweepAt = null;
let sweeping = false;
let resweepRequested = false;
let sweepTimer = null;

export function probeOnce(rawUrl) {
...
/** Test/shutdown hook: cancels the pending re-sweep timer sweep() always schedules. */
...
export function fetchFavicon(targetUrl, allowedHost, redirectsLeft) {
...
export async function handleIcon(res, id) {
...
export async function handlePut(req, res) {
  if (isReadOnly()) return sendJson(res, 403, { error: "read-only demo instance - changes aren't saved" });
...
export function sendFile(res, filePath, pathname, method) {
...
export function requestHandler(req, res) {
  ...
  if (pathname === "/api/health") return sendJson(res, 200, { ok: true, version: APP_VERSION, uptime: Math.round(process.uptime()), readOnly: isReadOnly() });
